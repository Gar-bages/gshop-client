  ...
  <div class="ratings">
    <div class="ratings-content">
      ...
      <RatingsFilter :selectType="selectType"
                     ...
                     @toggleOnlyContent="toggleOnlyContent"/> <!--vue自定义事件-->
  ...
  export default {
    data () {
      return {
        ...
        onlyContent:true, //只看有内容的
      }
    },
    mounted () {
      this.$store.dispatch('getShopRatings', () => { //异步请求
        this.$nextTick(() => {
          new BScroll('.ratings', {
            click: true
          })
    ...
    computed: {
      ...
      filterRatings () {
        ...
        return ratings.filter(rating => {
          ...
          //选的类型是全部评论（2） 全部评论里只看没有内容的 || 选的类型是全部评论（2） 全部评论里只看有内容的
          //选的类型是推荐评论（0） 推荐评论里只看没有内容的 || 选的类型是推荐评论（0） 推荐评论里只看有内容的
          //选的类型是吐槽评论（1） 吐槽评论里只看没有内容的 || 选的类型是吐槽评论（1） 吐槽评论里只看有内容的
